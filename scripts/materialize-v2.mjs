...
if (!/^v2-2c-[a-f0-9]{12}$/.test(manifest.version)) throw new Error('Invalid V2 version');
...
const expectedPackBytes = manifest.assets.reduce((sum, entry)=>sum+entry.bytes,0);
...
console.log(`Verified and materialized ${manifest.assets.length} registered V2 assets from ${parts.length} source chunks`);
