import type { AcquisitionJournal } from './card-acquisition.js';
...
export function applyCommand(session: BasicGameSession, catalog: BasicGameCatalog, command: GameCommand, acquisitions?: AcquisitionJournal): string | null {
  ...
  switch (command.type) {
    ...
    case "end-turn": return endTurn(session, Math.random, acquisitions);
    case "reveal-turn":
